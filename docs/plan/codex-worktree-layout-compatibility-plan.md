...
| Codex ID 与 4 位 repo 名完全相同 | 无法仅从路径区分两种布局 | 优先按 Codex 解释，使 `path` 和 `rm` 使用 ID |
...
4. `wk new` 的创建路径和原有行为不变。
5. `go test ./...` 和 `go build ./...` 通过。
...
- ✅ 使用真实 `~/worktrees` 验证 `ls`、`ls --all` 和 `path`。
